...
+ (void)updateProgressIndicator:(id *)indicator withProgress:(unsigned long long)progress ofTotal:(unsigned long long)total
{
	...
	//PDColoredProgressView *theProgress = (PDColoredProgressView *)indicator;
	//const unsigned int bytes = 1024 * 1024;
	//[theProgress setPl:[NSString stringWithFormat:@"%.2f MB of %.2f MB",(float)progress/bytes,(float)total/bytes ]];
...
- (BOOL)useDataFromCache
{
    ...
    NSLog(@"path in request file====%@",dataPath);
	if (!dataPath) {
		return NO;
	}
